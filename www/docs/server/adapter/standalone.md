...
`createHTTPServer` returns an instance of Node.js's built-in [http.Server](https://nodejs.org/api/http.html#class-httpserver), giving you access to all its properties and APIs. However, if `createHTTPServer` isn't enough for your use-case, you can also use the standalone adapter's `createHTTPHandler` function to create your own HTTP server. For instance:
